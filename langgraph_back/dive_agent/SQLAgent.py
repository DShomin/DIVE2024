        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    This db is postgres db, so pay attention when you generate a SQL query.
                    
                    Here are some examples:
                    1. What is the top selling product?
                    Answer: SELECT product_name, SUM(quantity) as total_quantity FROM sales WHERE product_name IS NOT NULL AND quantity IS NOT NULL GROUP BY product_name ORDER BY total_quantity DESC LIMIT 1

                    2. What is the total revenue for each product?
                    Answer: SELECT "product name", SUM(quantity * price) as total_revenue FROM sales WHERE "product name" IS NOT NULL AND quantity IS NOT NULL AND price IS NOT NULL GROUP BY "product name" ORDER BY total_revenue DESC

                    3. What is the market share of each product?
                    Answer: SELECT "product name", SUM(quantity) * 100.0 / (SELECT SUM(quantity) FROM sales) as market_share FROM sales WHERE "product name" IS NOT NULL AND quantity IS NOT NULL GROUP BY "product name" ORDER BY market_share DESC

                    4. Plot the distribution of income over time
                    Answer: SELECT income, COUNT(*) as count FROM users WHERE income IS NOT NULL GROUP BY income ORDER BY income
                    THE RESULTS SHOULD ONLY BE IN THE FOLLOWING FORMAT, SO MAKE SURE TO ONLY GIVE TWO ORE THREE COLUMNS:
                    [[x, y]]
                    or
                    [[label, x, y]]
                    ...
                    If you need to use UNION, you have to use bracket. for example:
...
""",
    ...
    def format_results(self, state: dict) -> dict:
        """Format query results into a human-readable format."""
        question = state["question"]
        if "error" in state:
